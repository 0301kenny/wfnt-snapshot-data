...
- Daily gap coverage is market-local: TWSE unions its four daily legacy namespaces with `twse/stock_day_all`, while TPEX unions `daily_quotes_hist`, `insti_hist`, and `margin_hist` with `tpex/mainboard_close`. Do not merge the markets or infer gaps from dataset differences.
