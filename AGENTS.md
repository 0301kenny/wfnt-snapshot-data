...
- Endpoint allowlist is only the 8 daily endpoints, 2 monthly revenue endpoints, the TDCC weekly endpoint, and the 3 backfill-only TWSE legacy endpoints declared in `scripts/endpoints.mjs`; the legacy endpoints must not enter the daily pipeline.
...
- TWSE legacy backfill raw snapshots use `data/raw/twse/{mi_index_hist,t86_hist,mi_margn_hist}/{yyyy}/{date}.json` and preserve official response bytes unchanged.
