...
- Endpoint allowlist is only the 8 daily endpoints, 2 monthly revenue endpoints, the TDCC weekly endpoint, and the 4 TWSE plus 4 TPEX backfill-only legacy endpoints declared in `scripts/endpoints.mjs`; the legacy endpoints must not enter the daily pipeline.
...
- Fundamentals derived outputs use the same pure numeric 6-character ID exclusion rule and combine daily TWSE/TPEX valuation with monthly TWSE/TPEX revenue.
